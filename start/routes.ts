...
Route.group(() => {
  ...
  Route.post('/item/:id/update', 'Web/MasterDataController.itemUpdate').as('master-data.item.update')
  Route.get('/item/:id/delete', 'Web/MasterDataController.itemDestroy').as('master-data.item.destroy')
})
  .prefix('/master-data')
  .middleware('auth')
  ...
  Route.group(() => {
    ...
    Route.get('/:idBuilding/:idRoom/:idItem/swing', 'Web/RemotesController.itemSwing').as('remote.item.swing')
  })
    .prefix('/remote')
    .middleware('auth')
